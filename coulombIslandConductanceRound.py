...
# How close around the maximum od we want to be
roundParam = .005;
# bias
bias = 0.0
...
epsilon = np.linspace(-4.0*capacitive, 4.0*capacitive, 1000);
...
for i in range(len(n0)):
	...
	maxLocation = np.where(transport==np.max(transport))[0];
	maxEpsilon = epsilon[maxLocation];

	roundEpsilon = np.linspace( maxEpsilon-roundParam, maxEpsilon+roundParam , 1000);

	transport = np.array([np.real(np.trace(T(eps))) for eps in roundEpsilon]);
	roundConduction = np.mean(transport); 
	print '%.9e\t%.9e\t%.9e' % (betaFraction[i], beta[i], roundConduction);
#toc
