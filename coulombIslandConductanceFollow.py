...
print >> sys.stderr, "Platform found is %s. Adding paths.\n" % platform
if platform == "linux2": 
...
if nEigenValue == 0:
	...
	maxLocation = capacitive-levels;
elif nEigenValue == 3:
	maxLocation = capacitive+levels;
else:
	raise Exception('Eigenvalue number must be 0-3.')

deltaEpsilon = 8.0*capacitive/1000.0 * 2.0;

epsilon = np.linspace( maxLocation - deltaEpsilon, maxLocation + deltaEpsilon, 1000 );
...
for i in range(len(n0)):
	...
	print '%.9e\t%.9e\t%.9e' % (betaFraction[i], beta[i], np.max(transport));
#toc
